 ...
 */
 //edited by Aidan Johnson on January 24th
int isKaprekar(int n) {

  if(n < 1) {
	//edit: replaced "false" with "0"
    return 0;
  }

  int i;
  long square = n * (long) n;
  //edit: defined "numDigits"
  int numDigits;
  //edit:set modulus to "1" instead of "0"
  long modulus = 1;
  long first, second;
  //edit:changed "log10(n)" to "log10(square)"
  numDigits = (int) log10(square) + 1;
  
  //for each possible "split" of the square...
  for(i=1; i<=numDigits; i++) {
    //increase the modulus by a factor of 10
    //edit: changed "moduolus" to "modulus"
	modulus *= 10;
